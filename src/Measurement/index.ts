...
export class Measurement<A extends Quantity> {
  ...
  toString = (): string => `${this.magnitude} ${this.symbol}`;
  ...
  sameUnit = <B extends Quantity>(b: Measurement<B>): boolean =>
    this.symbol === b.symbol &&
    this.baseRatio === b.baseRatio &&
    (this.quantity as Quantity) === b.quantity;

  times: MultiplyMeasurements<A> = (b) =>
    measurement(
      this.sameUnit(b) ? `${b.symbol}^2` : `${this.symbol}*${b.symbol}`,
  ...
  per: DivideMeasurements<A> = (b) =>
    measurement(
      this.sameUnit(b) ? '' : `${this.symbol}*(${b.symbol})^-1`,
      this.magnitude / b.magnitude,
      this.baseRatio / b.baseRatio,
